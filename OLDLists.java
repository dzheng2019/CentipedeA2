...
class GetAtXY<T> implements IListVisitor<IList<T>, T> {
  int xIndex;
  int yIndex;
  GetAtXY(int xIndex, int yIndex) {
    this.xIndex = xIndex;
    this.yIndex = yIndex;
  }
  @Override
  public T apply(IList<IList<T>> arg) {
    // TODO Auto-generated method stub
    return arg.accept(this);
  }
  @Override
  public T visitMt(MtList<IList<T>> mt) {
    // TODO Auto-generated method stub
    throw new RuntimeException(
        "Given y index is larger than the number of rows in grid");
    }
  @Override
  public T visitCons(ConsList<IList<T>> cons) {
    // TODO Auto-generated method stub
    if (yIndex < 0) {
      throw new RuntimeException("Given y index " + yIndex + 
          "is negative, which is not a valid row number in the grid");
    }
    else if (yIndex == 0) {
      return new GetAtX<T>(this.xIndex).apply(cons.first);
    }
    else {
      return new GetAtXY<T>(this.xIndex, this.yIndex - 1).apply(cons.rest);
    }
  }
...
class GetAtX<T> implements IListVisitor<T, T> {
  ...
  @Override
  public T apply(IList<T> arg) {
    // TODO Auto-generated method stub
    return arg.accept(this);
  }

  @Override
  public T visitMt(MtList<T> mt) {
    // TODO Auto-generated method stub
    throw new RuntimeException(
        "Given x index is larger than number of rows in grid");
  }

  @Override
  public T visitCons(ConsList<T> cons) {
    // TODO Auto-generated method stub
    // TODO Auto-generated method stub
    if (xIndex < 0) {
      throw new RuntimeException("Given y index " + xIndex + 
          "is negative, which is not a valid row number in the grid");
    }
    else if (xIndex == 0) {
      return cons.first;
      ...
      return new GetAtX<T>(this.xIndex - 1).apply(cons.rest);
    }
  }
