...
class Board {
    ...
    boolean tileCollisionLeft = 
        new OrMap<IList<ITile>>(
            new OrMapListPred<ITile>(
                new TileCollisionSolid (collideGridLocLeft))).apply(this.gameBoard);
  ...
  boolean collisionOccursRight(Posn collidePos, int size) {
    ...
    boolean tileCollisionRight = 
        new OrMap<IList<ITile>>(
            new OrMapListPred<ITile>(
                new TileCollisionSolid (collideGridLocRight))).apply(this.gameBoard);
  ...
  // determines if a collision has occurred in y direction 
  boolean collisionOccursY(Posn collidePos, int size) {
  ...
  boolean collisionOccurs(Posn collidePos, int size) {
    ...
    return this.collisionOccursY(collidePos, size)
        || this.collisionOccursRight(collidePos, size)
        || this.collisionOccursLeft(collidePos, size);
  }

  boolean softCollision(Posn collidePos, int size) {
    Posn collideGridLocRight =
        u.convertAbsoluteToGrid(
            new Posn(collidePos.x + size, collidePos.y), 
            this.units, this.row);
    ...
    boolean tileCollisionRight = 
        new OrMap<IList<ITile>>(
            new OrMapListPred<ITile>(
                new TileCollisionNonSolid (collideGridLocRight))).apply(this.gameBoard);
    ...
    boolean tileCollisionLeft = 
        new OrMap<IList<ITile>>(
            new OrMapListPred<ITile>(
                new TileCollisionNonSolid (collideGridLocLeft))).apply(this.gameBoard);

    return tileCollisionLeft || tileCollisionRight;
  }

  boolean inGnomeBoundingBox(Posn absLocation, Posn direction) {
    return u.convertAbsoluteToGrid(absLocation, this.units, this.row).y < 3;
  ...
  Board changeAtLocation(Posn location, boolean createPebble) {
    ...
    if (createPebble) {
      ITile dande = new GetAtXY<ITile>(gridLoc.x, gridLoc.y).apply(this.gameBoard);
      return new HitOrExplodeDande(this).apply(dande);
    }
    else {
      return new Board(
...
// checks if a position collides with a solid tile
class TileCollisionSolid implements IPred<ITile> {
  Posn collisionPoint;
  ...
  TileCollisionSolid (Posn collisionPoint) {
  ...
  public Boolean apply(ITile arg) {
     ...
     * arg.accept(IFuncTile<T> func) - T
     */
    return arg.collidesWithSolid(this.collisionPoint);
  }
}

//checks if a position collides with a non solide tile
class TileCollisionNonSolid implements IPred<ITile> {
  Posn collisionPoint;
  ...
  TileCollisionNonSolid (Posn collisionPoint) {
    this.collisionPoint = collisionPoint;
  }
  ...
  public Boolean apply(ITile arg) {
     ...
     * arg.accept(IFuncTile<T> func) - T
     */
    return arg.collidesWithNonSolid(this.collisionPoint);
  }
}
...
class HitOrExplodeDande implements IFuncTile<Board> {

  Board board;
  
  HitOrExplodeDande(Board board) {
    this.board = board;
  }
  
  @Override
  public Board apply(ITile arg) {
    // TODO Auto-generated method stub
    return arg.accept(this);
  }

  @Override
  public Board visitGrass(Grass grass) {
    // TODO Auto-generated method stub
    return this.board;
  }

  @Override
  public Board visitDandelions(Dandelions dande) {
    // TODO Auto-generated method stub
    if(dande.hp > 1) {
      return new ApplyAtXY(dande.gridLocation, 
          new ReduceDandelionHP()).apply(this.board);
    }
    else {
      // creates a pebble which is part of a pebble pile (hence true)
      return new ProducePebblePile(dande.gridLocation).apply(this.board);
    }
  }

  @Override
  public Board visitPebbles(Pebbles pebb) {
    // TODO Auto-generated method stub
    return this.board;
  }
